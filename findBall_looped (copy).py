...
def scan_area(_angleSearch):
    ...
    pic(path + 'camImage0.png', 0)
    commandAngles = motion.getAngles(names, useSensors)
    motionAngles.append(commandAngles)
    print str(commandAngles)
    motion.angleInterpolationWithSpeed("Head", [-maxAngleScan/2, 0.035], 0.1)
    pic(path + 'camImage1.png', 0)
    commandAngles = motion.getAngles(names, useSensors)
    motionAngles.append(commandAngles)
    print str(commandAngles)
    motion.angleInterpolationWithSpeed("Head", [0, 0.035], 0.1)
    pic(path + 'camImage2.png', 0)
    commandAngles = motion.getAngles(names, useSensors)
    motionAngles.append(commandAngles)
    print str(commandAngles)
    motion.angleInterpolationWithSpeed("Head", [maxAngleScan/2, 0.035], 0.1)
    pic(path + 'camImage3.png', 0)
    commandAngles = motion.getAngles(names, useSensors)
    motionAngles.append(commandAngles)
    print str(commandAngles)
    motion.angleInterpolationWithSpeed("Head", [maxAngleScan, 0.035], 0.1)
    pic(path + 'camImage4.png', 0)
    commandAngles = motion.getAngles(names, useSensors)
    motionAngles.append(commandAngles)
    print str(commandAngles)
    centers = analyze_img()
...
##        alpha = alpha.item()
    ...
    while cm[0] >= 0  and cm[0] < 400:
        ...
        alpha = (cm[1] - 320) * delta
##        alpha = alpha.item()
        motion.moveTo(0.15, 0, alpha*8/6)
        idx = idx + 1
    return 1
    ...
    while taskCompleteFlag == 0:
        ...
        if ballPosition==[0,0]:
            ...
            if lost == 0:
                ...
                taskCompleteFlag = walkDown(CoM, delta)
            else:
